...
} from "bpmn-js-properties-panel/lib/provider/camunda/element-templates/CreateHelper";
...
export default function CustomProps(
  ...
  var renderCustomField = function (id, p, idx) {
    ...
    if (propertyType === "Boolean") {
      entryOptions.widget = "checkbox";
    }

    if (propertyType === "String") {
      entryOptions.widget = "textField";
    }

    if (propertyType === "Text") {
      entryOptions.widget = "textBox";
    }

    if (propertyType === "Dropdown") {
      entryOptions.selectOptions = p.choices;
      entryOptions.widget = "selectBox";
    }
    entry = entryOptions;
    return entry;
  };
  ...
  if (template.scopes) {
    for (var scopeName in template.scopes) {
      ...
      scope.properties.forEach((p, idx) => {
 ...
 */
export function setPropertyValue(element, property, value, bpmnFactory) {
  var bo = getBusinessObject(element);
  ...
  if (bindingType === CAMUNDA_PROPERTY_TYPE) {
    ...
    if (!camundaProperties) {
      camundaProperties = elementHelper.createElement(
        "camunda:Properties",
        null,
        bo,
        bpmnFactory
 ...
 */
export function validateValue(value, property, translate) {
  ...
  if (pattern) {
    ...
    if (!matchesPattern(value, pattern)) {
      return (
        message ||
        translate("Must match pattern {pattern}", { pattern: pattern })
      );
    }
  }
}
