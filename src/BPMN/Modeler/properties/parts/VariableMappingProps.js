...
import extensionElementsEntry from "./implementation/ExtensionElements";
...
function getCamundaInOutMappings(element, type) {
  ...
  var signalEventDefinition = eventDefinitionHelper.getSignalEventDefinition(
    bo
  );
...
export default function VariableMappingProps(
  group,
  element,
  bpmnFactory,
  translate
  ...
  var getSelected = function (element, node) {
    ...
    var parameter = getVariableMappings(element, CAMUNDA_IN_EXTENSION_ELEMENT)[
      selection.idx
    ];

    if (!parameter && outEntry) {
      selection = outEntry.getSelected(element, parentNode);
      parameter = getVariableMappings(element, CAMUNDA_OUT_EXTENSION_ELEMENT)[
        selection.idx
      ];
    }
