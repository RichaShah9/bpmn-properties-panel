...
export default function Listener(element, bpmnFactory, options, translate) {
  ...
  function getSelectedListener(element, node) {
    var selection = (executionListenerEntry &&
      executionListenerEntry.getSelected(element, node)) || { idx: -1 };

    var listener = getListeners(bo, CAMUNDA_EXECUTION_LISTENER_ELEMENT)[
      selection.idx
    ];
  ...
  var setOptionLabelValue = function (type) {
    return function (element, node, option, property, value, idx) {
      ...
      var listenerType = ImplementationTypeHelper.getImplementationType(
        listener
      );
